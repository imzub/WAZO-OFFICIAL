...
- Website checkout is `Master`; the refreshed v2 installer replacement is
  staged on `Master` and will be live-verified from the new Pages deployment
  after publication.
- The v2.0.0.0 testing installer replacement is sourced from private WAZO
  ...
  Cache-busted public bytes match this record exactly through Pages deployment
  `31394023056` from website commit `d91d1d3`; verified at
  `2026-08-10T13:45:29Z`.
...
- The current optional testing installer is WAZO `v2.0.0` / build `2.0.0.0`, 99,433,175 bytes, SHA-256 `248C263FA712E40AF6835E7902D7C31B7ACFDBB90210ED3980670DF264365B12`, from WAZO `Dev/QA` source commit `3d563d1`. The prior baseline was 1646/1646; this post-fix slice passed focused source/UI contracts 316/316, static/documentation checks, packaged installer verification, and full public download byte/hash verification. The bounded complete-suite rerun exceeded 120 seconds without a result, so no new full-suite claim is made. Live byte/size verification passed through Pages deployment `31394023056`; v1.1.7 remains stable and the package is unsigned.
